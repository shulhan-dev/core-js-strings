...
function repeatString(str, times) {
  return str.repeat(times < 0 ? str : times);
}
...
function sumOfCodes(str = '') {
  ...
  return str.split('').reduce((acc, el) => el.charCodeAt() + acc, 0);
}
...
function isPalindrome(str) {
  const clearStr = str.toLowerCase().replace(/[^a-z]/g, '');
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((char) =>
      alphabet.indexOf(char) !== -1 ? encoded.at(alphabet.indexOf(char)) : char
    )
    .join('');
}
